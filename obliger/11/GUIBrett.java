...
class GUIBrett extends JFrame {
	...
	GUIBrett(SudokuBeholder beholder, int boksRader, int boksKolonner) {
		...
		this.knappLosninger.setText("Neste loesning");
	...
	// Oppdater antall loesninger
	private void oppdaterAntallLosninger() {
		int antall = this.beholder.hentAntallLosninger();

		// Tekst
		if (antall == 0)
			this.antallLosninger.setText("Ingen loesninger");
		if (antall > 0)
			this.antallLosninger.setText("Loesning: " + (this.losningsIndeks+1) + "/" + this.beholder.hentAntallLosninger());
